...
function openBrowPreview(key){
  ...
  const src=`./assets/brow-${fileKey}-${selectedBrowThickness}.jpg?v=25`;
  analysisPreviewVisual.innerHTML=`<img class="preview-raster brows" src="${src}" alt="${d.title}, épaisseur ${thicknessLabel(selectedBrowThickness)}">`;
...
function openLookPreview(key){
  ...
  if(!d)return;
  analysisPreviewTitle.textContent=d.title;
  const src=`./assets/makeup-${key}.jpg?v=25`;
  analysisPreviewVisual.innerHTML=`<img class="preview-raster makeup" src="${src}" alt="Référence visuelle ${d.title}">`;
