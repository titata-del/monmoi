...
function rgbToLab([r,g,b]){
  ...
  let z=r*.0193339+g*.1191920+b*.9503041;
  x/=0.95047; y/=1.00000; z/=1.08883;
  const f=t=>t>0.008856?Math.cbrt(t):(7.787*t)+(16/116);
  const fx=f(x),fy=f(y),fz=f(z);
  return [116*fy-16,500*(fx-fy),200*(fy-fz)];
}
function colorimetryProfile(skin,iris,brows){
  const [L,a,b]=rgbToLab(skin);
  const C=Math.hypot(a,b);
  let temperature="neutre";
  let tempScore=0;
  if(b>18 && a>7){temperature="chaude";tempScore=Math.min(1,(b-18)/18+.25)}
  if(b<12 && a>8){temperature="froide";tempScore=Math.min(1,(12-b)/10+.25)}
  if(b>14 && a<7){temperature="olive";tempScore=Math.min(1,(7-a)/7+.25)}
  if(Math.abs(b-15)<4 && a>=7&&a<=14){temperature="neutre";tempScore=.55}

  const depth=L>=72?"claire":L>=58?"moyenne":L>=43?"mate":"profonde";
  const chroma=C>=32?"vive":C>=21?"modérée":"douce";
  const contrastValue=Math.max(
    Math.abs(lum(skin)-lum(iris)),
    Math.abs(lum(skin)-lum(brows))
  );
  const contrast=contrastValue>=105?"fort":contrastValue>=62?"moyen":"doux";

  let family="";
  if(temperature==="chaude"){
    family=depth==="claire"?(chroma==="vive"?"chaud lumineux":"chaud doux"):(depth==="profonde"?"chaud profond":"chaud équilibré");
  }else if(temperature==="froide"){
    family=depth==="claire"?(chroma==="vive"?"froid lumineux":"froid doux"):(depth==="profonde"?"froid profond":"froid équilibré");
  }else if(temperature==="olive"){
    family=depth==="profonde"?"olive profond":chroma==="douce"?"olive doux":"olive équilibré";
  }else{
    family=contrast==="fort"?"neutre contrasté":chroma==="douce"?"neutre doux":"neutre équilibré";
  }
  return{temperature,depth,chroma,contrast,family,lab:[L,a,b],confidence:tempScore};
}
function paletteForProfile(p){
  const palettes={
    "chaud lumineux":{
      daily:["#C9846B","#D7A07B","#A97455","#E3B19A","#8E5A42"],
      signature:["#D46F4C","#C68A2D","#8A6A32","#B75A3C","#E89C5F"],
      evening:["#7A3E2C","#8E5B31","#B66A44","#5E3C2D","#D17C56"],
      blush:["#D98778","#C97166"],bronzer:["#9C6545","#B47A56"],lips:["#B85F50","#C97968"],eyes:["#8A613F","#B08A5A"]
    },
    "chaud doux":{
      daily:["#C98E7D","#CFA48E","#A98270","#D9B29D","#9A7161"],
      signature:["#B97464","#B88C63","#9A7B61","#C58F76","#A86C5C"],
      evening:["#765146","#8C6757","#9A6652","#61473F","#A97964"],
      blush:["#C9827B","#B97872"],bronzer:["#99705A","#AD7D61"],lips:["#A96160","#B77A70"],eyes:["#806657","#9A7E68"]
    },
    "froid lumineux":{
      daily:["#C982A4","#D19BB7","#987392","#D9B2CA","#7F607F"],
      signature:["#B95185","#8C5AA6","#C15F97","#6B5FA7","#D076A9"],
      evening:["#704263","#59406F","#8A4D79","#3F3959","#9D5C86"],
      blush:["#C96F9A","#B85E8A"],bronzer:["#887077","#9B7C83"],lips:["#A94E78","#C36692"],eyes:["#67516F","#80658C"]
    },
    "froid doux":{
      daily:["#B9869D","#C29CAF","#8E7384","#D0B0BF","#7B6874"],
      signature:["#A56F88","#8C728F","#B37C98","#766A88","#C18EA6"],
      evening:["#694E61","#5B5068","#7D5D72","#46404F","#8B687D"],
      blush:["#B77894","#A96C87"],bronzer:["#847078","#967D85"],lips:["#9E6178","#AF778C"],eyes:["#6D5D6B","#7F6D7D"]
    },
    "olive équilibré":{
      daily:["#B9876B","#C49A79","#8F7A5B","#D0AA8A","#806B52"],
      signature:["#A86F49","#8A7A43","#B4834F","#7E6940","#C49361"],
      evening:["#65533A","#6F4D3A","#806346","#4F4938","#906246"],
      blush:["#B97970","#A86C65"],bronzer:["#8A694B","#9B7653"],lips:["#985F55","#A86F60"],eyes:["#6E6748","#827957"]
    },
    "olive doux":{
      daily:["#B6927C","#C2A08A","#91806B","#CCAE98","#827366"],
      signature:["#9C7861","#8E805F","#A8866B","#7B7359","#B49779"],
      evening:["#665A4F","#5D594A","#776553","#4A4740","#806A58"],
      blush:["#AD7E79","#9D736F"],bronzer:["#88705A","#987A61"],lips:["#916761","#A1766D"],eyes:["#6D695B","#7A7463"]
    },
    "neutre contrasté":{
      daily:["#B97A82","#C49B99","#8A6F70","#D0B0AA","#705B61"],
      signature:["#A94D63","#8A5570","#B96372","#6D5A77","#C17781"],
      evening:["#5C3947","#493D50","#704654","#36323F","#7F505E"],
      blush:["#B96E7B","#AA626F"],bronzer:["#806762","#92746E"],lips:["#9A4D61","#AF6071"],eyes:["#5E535A","#71636C"]
    },
    "neutre doux":{
      daily:["#B98E8B","#C2A4A0","#8D7775","#CDB3AE","#7F696A"],
      signature:["#A67276","#93727C","#B17D7F","#806E79","#BA8D8A"],
      evening:["#675458","#5A5058","#755D61","#484247","#80676A"],
      blush:["#AD7B7E","#A16F73"],bronzer:["#88706A","#987B74"],lips:["#94666A","#A77576"],eyes:["#6C5F62","#7B6D70"]
    }
  };
  const fallback={
    daily:["#C08A82","#B89A90","#8D776F","#D1AEA0","#7D675F"],
    signature:["#A76469","#9A746E","#B37472","#846B73","#C0877D"],
    evening:["#654B50","#5B4D55","#73585C","#463F46","#806165"],
    blush:["#B8797B","#A86F72"],bronzer:["#876E66","#977A70"],lips:["#976267","#A87376"],eyes:["#6B5B60","#7B6B70"]
  };
  return palettes[p.family]||fallback;
}
...
function combineColorFrames(frames){
  ...
  const profile=colorimetryProfile(skin,iris,brows);
  const paletteProfile=paletteForProfile(profile);
  ...
  return{
    ...
    undertone:profile.temperature,
    contrast:profile.contrast,
    colorDepth:profile.depth,
    colorChroma:profile.chroma,
    colorFamily:profile.family,
    colorConfidence:profile.confidence,
    paletteProfile
  };
}
...
function updateAll(a){
  ...
  $("#aColor").innerHTML=`Peau ${colorPill(a.skinName,a.skinHex)} · famille <strong>${a.colorFamily}</strong>.`;
  $("#colorTemperature").textContent=a.undertone;
  $("#colorDepth").textContent=a.colorDepth;
  $("#colorChroma").textContent=a.colorChroma;
  $("#colorContrast").textContent=a.contrast;
  $("#colorFamily").textContent=`Profil détecté : ${a.colorFamily}. Cette lecture combine température, profondeur de peau, intensité chromatique et contraste visage/yeux/sourcils.`;
  renderColorPalettes(a);
...
function colorAdvice(a){
  if(a.undertone==="chaude") return "Sous-ton chaud : pêche, abricot, terracotta doux, caramel, bronze doré et bruns chauds sont prioritaires. Les roses bleutés très froids peuvent paraître plus durs.";
  if(a.undertone==="froide") return "Sous-ton froid : rose froid, bois de rose, mauve, prune douce, taupe froid et bruns cendrés sont prioritaires. Les oranges très chauds sont moins naturels.";
  if(a.undertone==="olive") return "Sous-ton olive : terre cuite sourde, bronze neutre, rose brun, pêche atténué, kaki doux et taupe olive donnent généralement le meilleur équilibre.";
  return "Sous-ton neutre : tu peux utiliser les deux familles, mais les teintes modérément saturées proches de ton niveau de contraste seront les plus harmonieuses.";
}
function renderSwatches(target,colors){
  const el=$(target);
  if(!el)return;
  el.innerHTML=colors.map(c=>`<span class="palette-swatch" style="background:${c}" title="${c}"></span>`).join("");
}
function renderColorPalettes(a){
  const p=a.paletteProfile||paletteForProfile({family:a.colorFamily});
  renderSwatches("#paletteDaily",p.daily);
  renderSwatches("#paletteSignature",p.signature);
  renderSwatches("#paletteEvening",p.evening);
  const cats=[
    ["Blush",p.blush],["Bronzer",p.bronzer],["Lèvres",p.lips],["Yeux",p.eyes]
  ];
  $("#paletteCategories").innerHTML=cats.map(([name,colors])=>`
    <div class="category-palette">
      <span>${name}</span>
      <div class="palette">${colors.map(c=>`<span class="palette-swatch" style="background:${c}"></span>`).join("")}</div>
    </div>`).join("");
...
function recommendPalette(a){return (a.paletteProfile||paletteForProfile({family:a.colorFamily})).daily.slice(0,4);}
...
const BROW_PREVIEW_IMAGES={
  recommended:{thin:"./assets/3d/brow-recommended-thin.jpg?v=28",medium:"./assets/3d/brow-recommended-medium.jpg?v=28",thick:"./assets/3d/brow-recommended-thick.jpg?v=28","very-thick":"./assets/3d/brow-recommended-very-thick.jpg?v=28"},
  straight:{thin:"./assets/3d/brow-straight-thin.jpg?v=28",medium:"./assets/3d/brow-straight-medium.jpg?v=28",thick:"./assets/3d/brow-straight-thick.jpg?v=28","very-thick":"./assets/3d/brow-straight-very-thick.jpg?v=28"},
  soft:{thin:"./assets/3d/brow-soft-thin.jpg?v=28",medium:"./assets/3d/brow-soft-medium.jpg?v=28",thick:"./assets/3d/brow-soft-thick.jpg?v=28","very-thick":"./assets/3d/brow-soft-very-thick.jpg?v=28"},
  natural:{thin:"./assets/3d/brow-natural-thin.jpg?v=28",medium:"./assets/3d/brow-natural-medium.jpg?v=28",thick:"./assets/3d/brow-natural-thick.jpg?v=28","very-thick":"./assets/3d/brow-natural-very-thick.jpg?v=28"},
  lift:{thin:"./assets/3d/brow-lift-thin.jpg?v=28",medium:"./assets/3d/brow-lift-medium.jpg?v=28",thick:"./assets/3d/brow-lift-thick.jpg?v=28","very-thick":"./assets/3d/brow-lift-very-thick.jpg?v=28"}
};
const MAKEUP_PREVIEW_IMAGES={
  soft:"./assets/3d/makeup-soft.jpg?v=28",
  latte:"./assets/3d/makeup-latte.jpg?v=28",
  clean:"./assets/3d/makeup-clean.jpg?v=28",
  siren:"./assets/3d/makeup-siren.jpg?v=28",
  douyin:"./assets/3d/makeup-douyin.jpg?v=28",
  bronzy:"./assets/3d/makeup-bronzy.jpg?v=28",
  cold:"./assets/3d/makeup-cold.jpg?v=28",
  nomakeup:"./assets/3d/makeup-nomakeup.jpg?v=28"
};
...
function openBrowPreview(key){
  ...
  const fileKey=key==="recommended"?"recommended":d.kind;
  const src=BROW_PREVIEW_IMAGES?.[fileKey]?.[selectedBrowThickness]||"";
  analysisPreviewVisual.innerHTML=src
    ? `<img class="preview-raster brows" src="${src}" alt="${d.title}, épaisseur ${thicknessLabel(selectedBrowThickness)}">`
    : `<div class="preview-image-fallback">Aperçu sourcils indisponible</div>`;
  analysisPreviewText.textContent=d.text+" Épaisseur affichée : "+thicknessLabel(selectedBrowThickness)+". La photo change réellement avec la forme et l’épaisseur sélectionnées.";
  analysisPreviewTag.textContent="Référence forme + épaisseur";
  showAnalysisPreview();
}
...
function openLookPreview(key){
  ...
  if(!d)return;
  analysisPreviewTitle.textContent=d.title;
  const src=MAKEUP_PREVIEW_IMAGES?.[key]||"";
  analysisPreviewVisual.innerHTML=src
    ? `<img class="preview-raster makeup" src="${src}" alt="Référence visuelle ${d.title}">`
    : `<div class="preview-image-fallback">Aperçu makeup indisponible</div>`;
  analysisPreviewText.textContent=d.text+" L’image affichée est spécifique à ce style afin que tu voies immédiatement ce qui change.";
  analysisPreviewTag.textContent="Référence du style sélectionné";
  showAnalysisPreview();
}
